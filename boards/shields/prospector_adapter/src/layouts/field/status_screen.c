...
lv_obj_t *zmk_display_status_screen() {
    ...
    lv_obj_set_size(zmk_widget_line_segments_obj(&line_segments_widget), 320, 150);
    ...
    lv_obj_align(zmk_widget_layer_label_obj(&layer_label_widget), LV_ALIGN_TOP_LEFT, 9, 10);

    zmk_widget_output_init(&output_widget, screen);
    lv_obj_align(zmk_widget_output_obj(&output_widget), LV_ALIGN_TOP_LEFT, 4, 48);

    zmk_widget_battery_label_init(&battery_label_widget, screen);
    lv_obj_align(zmk_widget_battery_label_obj(&battery_label_widget), LV_ALIGN_BOTTOM_LEFT, 9, -12);
